...
'''# Save correlation to compare :D
metrics = {}
metrics["correlation"] = np.corrcoef(predictions, test_labels)[0,1]
metrics['spearman'] = stats.spearmanr(predictions, test_labels).correlation'''
...
# Pour chaque tranche de jours: 30, 90, ..., 360 and 1825 days.
...
try:
    ...
    plt.savefig(os.path.join(path_result, "roc_curve.png"), bbox_inches="tight")
    plt.close()
except:
...
print("len pred", pred_theo)
print("len lab", test_labels_theo)
print("len ind", indice_theo)
resu_pred = pd.DataFrame({"pred":pred_theo,
                        "lab":test_labels_theo,
                        "ind":indice_theo})
resu_pred.to_csv("resu_pred_THEO.csv", index=False)
...
try:
    for day_threshold in [30,90,180,270,360,720,1080,1440,1825]:
        ...
        plt.savefig(os.path.join(path_result, "confusion_matrix_" + str(day_threshold) + "days.png"), bbox_inches="tight")
        plt.close()
except:
