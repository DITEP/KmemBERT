...
class EHRDataset(Dataset):
    ...
    def __init__(self, path_dataset, config, train=True, df=None, return_id=False):
        ...
        # On commente cette ligne pour faire tourner le code
        ...
        # Modification du labels : score de survie -> 0 ou 1
        #self.labels = time_survival_to_label(self.survival_times, self.mean_time_survival)
        self.labels = np.array(list(self.df['State']))
...
class PredictionsDataset(EHRDataset):
    ...
    def __init__(self, *args, device=None, output_hidden_states=False, return_id=False, **kwargs):
        ...
        # Ajout indice pour suivre les predictions
        self.indice_followup = np.array(list(self.df.MyIndice))

        # Ajout FLAG_lastRecent pour faire prediction sur le dernier CR
        self.FLAG_most_Recent = np.array(list(self.df.FLAG_mostRecent))
    ...
    def __getitem__(self, index):
        ...
        # Get (Noigr, indices of CR for this given Noigr)
        noigr, k = self.index_to_ehrs[index]

        # Get indices to select dt, labels.
        indices = self.noigr_to_indices[noigr][:k][-self.config.max_ehrs:]
        dt = self.survival_times[indices] - min(self.survival_times[indices])

        # Recuperer indice lors de la ligne actuelle, [-1] pour garder le dernier element de la liste
        ind_raw = self.indice_followup[indices][-1]

        # Recuperer FLAG lors de la ligne actuelle, [-1] pour garder le dernier element de la liste
        flag_recent = self.FLAG_most_Recent[indices][-1]

        # Get label
        label = min(self.labels[indices])
        label = self.labels[indices][-1]

        # Get Input for T2 = Output of BASE
        outputs = self.noigr_to_outputs[noigr][:k][-self.config.max_ehrs:]

        # Print to test if everything is fine
        #print("Noigr: ", noigr, " | Survival Time: ", self.survival_times[indices], " | IndiceFollowUp: ", ind_raw, "\n")

        if self.output_hidden_states:
            outputs = torch.tensor(outputs).type(torch.float32)
            return (outputs[:,0,:], dt, label, ind_raw, flag_recent)
